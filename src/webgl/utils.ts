...
export function MakeRotate(deg: float32 = 0, axis: math.AXIS = math.AXIS.X): matrix.Matrix {
    const r: matrix.Matrix = matrix.MakeIdentity();

    const rad: float32 = deg * math.DEG_2_RAD;
...
export function MakeProjection(fovx_deg: float32, aspectRatio: float32, near: float32, far: float32): matrix.Matrix {
    var zoom_x: float32 = 1.0 / Math.tan(fovx_deg * math.DEG_2_RAD / 2.0);
