...
export interface IEntity {
     ...
     */
    dryGet<T extends Component>(cc: ComponentConstructor<T>): T;
...
export class Entity implements IEntity {
    ...
    public dryGet<T extends Component>(cc: ComponentConstructor<T> & ComponentType): T {
        let idx: uint = cc.IDX;
        if (Component.IDX === idx) {
            throw "error";
        }
    ...
    public each(): void {
        for (const com of this._component_map) {
            com.action();
        }
    }
}
